...
class SudokuGame {
    ...
    init() {
        try {
            ...
            alert("Erreur d'initialisation du jeu. Une nouvelle partie va être lancée.");
            this.startNewGame();
        }
    }
    ...
    setupEventListeners() {
        ...
        if (this.dom.eraseBtn) this.dom.eraseBtn.addEventListener('click', () => this.handleInput(0));
        ...
        if (this.dom.createProfileBtn) {
            this.dom.createProfileBtn.addEventListener('click', () => this.createNewProfile());
        }
        ...
        if (this.dom.newSaveBtn) {
            this.dom.newSaveBtn.addEventListener('click', () => this.promptSaveGame());
        }

        if (this.dom.saveNameConfirm) {
            this.dom.saveNameConfirm.addEventListener('click', () => this.confirmSaveGame());
        }
        ...
        document.addEventListener('click', (e) => {
            if (this.dom.profileBtn && this.dom.profileDropdown) {
                if (!this.dom.profileBtn.contains(e.target) && !this.dom.profileDropdown.contains(e.target)) {
                    this.dom.profileDropdown.classList.add('hidden');
                }
            }
        });
        ...
        if (this.dom.createRoomChoiceBtn) {
            this.dom.createRoomChoiceBtn.addEventListener('click', () => {
                ...
                if (this.dom.battleNameInput) {
                    this.dom.battleNameInput.value = this.profileManager.currentProfile || '';
                }
            });
        }
        ...
        if (this.dom.joinRoomChoiceBtn) {
            this.dom.joinRoomChoiceBtn.addEventListener('click', () => {
                ...
                if (this.dom.joinNameInput) {
                    this.dom.joinNameInput.value = this.profileManager.currentProfile || '';
                }
            });
        }
        ...
        if (this.dom.confirmJoinRoomBtn) {
            this.dom.confirmJoinRoomBtn.addEventListener('click', () => this.joinBattleRoom());
        }
    ...
    renderCell(index) {
        ...
        if (this.selectedCell && parseInt(this.selectedCell.dataset.index) === index) {
    ...
    loadGame() {
        ...
        if (saved) {
            try {
                ...
                if (!state || !state.board || !state.initialBoard || !state.solution || !Array.isArray(state.notes)) {
                ...
                if (diffDisplay) {
                    diffDisplay.textContent = this.difficulty.charAt(0).toUpperCase() + this.difficulty.slice(1);
                }
            } catch (e) {
    ...
    saveToSlot(slotId, slotName) {
        ...
        if (!slots[slotId] && Object.keys(slots).length >= 5) {
            return { success: false, error: "Limite de 5 sauvegardes atteinte. Supprimez une sauvegarde existante." };
        }
    ...
    loadFromSlot(slotId) {
        ...
        try {
            ...
            if (!slot.board || !slot.initialBoard || !slot.solution || !Array.isArray(slot.notes)) {
    ...
    selectCell(cell) {
        ...
        if (val && !cell.querySelector('.notes-grid')) { // Don't highlight if it's just notes
            document.querySelectorAll('.cell').forEach(c => {
                if (c.textContent === val && !c.querySelector('.notes-grid')) c.classList.add('highlighted');
            });
        }
    }
    ...
    gameWon() {
        ...
        const isNewRecord = this.statsManager.recordGameEnd(this.difficulty, this.timer, true);
    ...
    showStats() {
        ...
        // Update records
        document.getElementById('record-easy').textContent =
            records.easy ? this.statsManager.formatTime(records.easy) : '--:--';
        document.getElementById('record-medium').textContent =
            records.medium ? this.statsManager.formatTime(records.medium) : '--:--';
        document.getElementById('record-hard').textContent =
            records.hard ? this.statsManager.formatTime(records.hard) : '--:--';
        document.getElementById('record-expert').textContent =
            records.expert ? this.statsManager.formatTime(records.expert) : '--:--';
        ...
        stats.averageTime > 0 ? this.statsManager.formatTime(stats.averageTime) : '--:--';
    ...
    showSaves() {
        ...
        if (slotIds.length === 0) {
            ...
            emptyMessage.innerHTML = '📁 Aucune sauvegarde<br><span style="font-size: 0.9em; opacity: 0.7;">Créez une nouvelle sauvegarde pour commencer</span>';
            ...
            slotIds.forEach(slotId => {
                ...
                card.innerHTML = `
                    <div class="save-card-header">
                        ...
                        <div class="save-card-difficulty" style="--diff-color: ${difficultyColors[slot.difficulty]}">
                    ...
                    <div class="save-card-info">
                        <div class="save-card-stat">
                            ...
                            <span class="save-card-value">${this.formatTime(slot.seconds)}</span>
                        </div>
                        <div class="save-card-stat">
                            <span class="save-card-label">📊 Progression:</span>
                            <span class="save-card-value">${slot.progression}%</span>
                        </div>
                        <div class="save-card-stat">
                            <span class="save-card-label">📅 Date:</span>
                            <span class="save-card-value">${this.formatDate(slot.timestamp)}</span>
                        </div>
                    </div>
            ...
            this.dom.savesList.addEventListener('click', (e) => {
                ...
                if (target.classList.contains('delete-btn')) {
                    ...
                    if (slot) {
                        this.showConfirm(`Êtes-vous sûr de vouloir supprimer "${slot.name}" ?`, () => {
    ...
    showConfirm(message, onConfirm) {
        ...
        this.dom.confirmCancel.parentNode.replaceChild(newCancelBtn, this.dom.confirmCancel);
    ...
    promptSaveGame() {
        ...
        if (Object.keys(slots).length >= 5) {
            alert('Limite de 5 sauvegardes atteinte. Supprimez une sauvegarde existante.');
            return;
        }

        if (this.dom.saveNameModal && this.dom.saveNameInput) {
            this.dom.saveNameInput.value = `Partie ${new Date().toLocaleDateString('fr-FR')}`;
    ...
    async createBattleRoom() {
        ...
        const difficulty = selectedDiff ? selectedDiff.dataset.battleDiff : 'medium';
    ...
    updateProfileUI() {
        if (this.dom.profileName) {
            this.dom.profileName.textContent = this.profileManager.getCurrentProfile();
        }
...
class SoundManager {
    ...
    playTap() { this.playTone(600, 'sine', 0.1, 0.1); }
    playNote() { this.playTone(800, 'sine', 0.05, 0.05); }
    playErase() { this.playTone(300, 'sine', 0.15, 0.1); }
    playWin() {
...
class MediaController {
    ...
    init() {
        ...
        if (this.closeBtn) this.closeBtn.addEventListener('click', () => this.toggleMedia());
        if (this.loadBtn) this.loadBtn.addEventListener('click', () => this.loadMedia());
        if (this.detachBtn) this.detachBtn.addEventListener('click', () => this.toggleFloat());
...
class ProfileManager {
    ...
    init() {
        ...
        const hasOldData = localStorage.getItem('sudoku-save') || localStorage.getItem('sudoku-stats');
        ...
        if (!profilesExist) {
            ...
            this.currentProfile = localStorage.getItem('sudoku-current-profile') || this.profiles[0];
        }
    }
    ...
    deleteProfile(name) {
        if (this.profiles.length === 1) {
            return { success: false, error: 'Impossible de supprimer le dernier profil' };
        }
